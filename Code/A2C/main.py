...
from agent import A2CAgent, RolloutBuffer


def init_parameters():
    """初始化命令行参数"""
    parser = argparse.ArgumentParser(description="A2C for CarRacing-v3")
    ...
    # A2C参数（相比PPO更小的n_steps，无n_epochs）
    parser.add_argument("--n_steps", type=int, default=256, help="Steps per rollout (A2C用更小的值，适合2-3M参数的CNN网络)")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate (根据2-3M参数的CNN网络调整，CNN需要更稳定的训练)")
...
# make_env, save_model, plot_and_save_rewards 现在从common模块导入
...
def main():
    ...
    print("=" * 60)
    print("A2C Training for CarRacing-v3")
    print("=" * 60)
    ...
    # 使用统一进度条
    pbar = create_training_progress_bar(args.total_timesteps, "A2C")
            ...
            for i in range(args.num_envs):
                if dones[i]:
                    ...
                    if len(episode_rewards) % args.save_episode_freq == 0:
                        save_path = save_model(agent.network, os.path.join(current_path, "models"), f"a2c_checkpoint_ep{len(episode_rewards)}")
    ...
    # 保存最终模型
    final_path = save_model(agent.network, os.path.join(current_path, "models"), "a2c_final")
    ...
    if episode_rewards:
        try:
            ...
            save_training_data_csv(data_dict, plot_path, "a2c")
