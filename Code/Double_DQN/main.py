...
def init_parameters():
    parser = argparse.ArgumentParser(description="Double DQN for CarRacing-v3")
...
def main():
    ...
    print("=" * 60)
    print("Double DQN Training for CarRacing-v3")
    print("=" * 60)
    ...
    pbar = create_training_progress_bar(args.total_timesteps, "Double-DQN")
    ...
    while total_steps < args.total_timesteps:
        ...
        for i in range(args.num_envs):
            agent.buffer.push(obs[i], actions[i], next_obs[i], rewards[i], dones[i])
            ...
            if dones[i]:
                ...
                if len(episode_rewards) % args.save_episode_freq == 0:
                    save_model(agent.q_net, models_dir, f"ddqn_checkpoint_ep{len(episode_rewards)}")
    ...
    save_model(agent.q_net, models_dir, "ddqn_final")
    ...
    if episode_rewards:
        ...
        save_training_data_csv(data_dict, plots_dir, "ddqn")
