...
def visualize(args):
    ...
    print(f"Episodes: {args.episodes}, Frame Skip: {args.frame_skip}")
    ...
    env = make_sota_env(render_mode="human", frame_stack=4, frame_skip=1)
    ...
    # 5. 运行 Loop - 连续跑多个赛道
    total_rewards = []
    # 决策频率：如果指定了 decision_freq，使用它；否则使用训练时的 frame_skip
    decision_interval = args.decision_freq if args.decision_freq else args.frame_skip
