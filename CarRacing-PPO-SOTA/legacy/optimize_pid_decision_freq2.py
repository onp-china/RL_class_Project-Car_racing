...
def evaluate_params(args_tuple):
    ...
    返回: (混合评分, 原始奖励, 平滑度惩罚)
    """
    pid_params, model_path, decision_interval = args_tuple
    kp, ki, kd = pid_params
    
    # 初始化环境 (无渲染模式，frame_skip=1 以便高频控制)
    env = make_sota_env(render_mode=None, frame_stack=4, frame_skip=1)
    ...
    while not (done or truncated) and steps < 1000: # 限制最大步数防止死循环
...
# --- 3. 随机搜索主循环 ---
def optimize(args):
    decision_interval = args.decision_freq if args.decision_freq else args.frame_skip
    print(f"开始 PID 参数搜索... (Model: {args.model})")
    print(f"控制模式：高频 PID (模型每 {decision_interval} 帧决策，PID 每帧执行)")
    print(f"目标：最大化 (Reward - 5.0 * Jerk)")
    print(f"注意：由于控制频率提高，Kp 搜索范围调整为 [0.05, 0.8]")
    
    best_score = -float('inf')
    best_params = (0.3, 0.0, 0.1) # 默认初始值
    best_reward = 0
    best_jerk = 0
    
    # 搜索配置
    num_trials = 30  # 尝试多少组参数
    ...
    for i in range(num_trials):
        ...
        # 评估 (运行 1 次，为了速度)
        decision_interval = args.decision_freq if args.decision_freq else args.frame_skip
        s, r, j = evaluate_params((params, args.model, decision_interval))
    ...
    print("\n您可以直接使用以下命令进行可视化：")
    print(f"python visualize_agent.py --episodes 5 --use_pid --pid_kp {best_params[0]:.4f} --pid_ki {best_params[1]:.4f} --pid_kd {best_params[2]:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--decision_freq", type=int, default=None,
                       help="模型决策频率（每N帧决策一次，None=使用frame_skip，2=折中方案）")
